...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
      ...
      while ($continue === true) {
        ini_set('max_execution_time', 180); //3 minutes
        $url = "https://private-anon-5cfa329aee-jikan.apiary-proxy.com/v3/top/manga/".$i."/manga";
        ...
        $resp = curl_exec($curl);
        curl_close($curl);
        if (isset(json_decode($resp)->status) &&  json_decode($resp)->status == 404) {
          $continue = false;
        }



        if (isset(json_decode($resp)->top)) {
          $mangas = json_decode($resp)->top;
          foreach ($mangas as $key => $manga) {
            if (empty(Manga::where('id', $manga->mal_id)->first())) {
              $manga = Manga::create([
                'id' => $manga->mal_id,
                'nom' => $manga->title,
                'img' => $manga->image_url,
